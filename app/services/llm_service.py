"""LLM 服务 - 使用 OpenAI SDK 统一调用 DeepSeek 和 FastGPT"""
import os
...
# FastGPT 客户端（OpenAI SDK 兼容，用 APP_ID 作为 model）
fastgpt_client = None
if settings.FASTGPT_API_KEY and settings.FASTGPT_APP_ID != "your_app_id_here":
...
def chat_deepseek(messages: list[dict], temperature: float = 0.7) -> str:
...
def chat_fastgpt(messages: list[dict], temperature: float = 0.3) -> str:
    """使用 OpenAI SDK 调用 FastGPT 知识库进行 RAG 对话"""
    if not fastgpt_client:
        raise RuntimeError("FastGPT 未配置，请检查环境变量 FASTGPT_API_KEY 和 FASTGPT_APP_ID")
    completion = fastgpt_client.chat.completions.create(
        model=settings.FASTGPT_APP_ID,  # FastGPT 用 APP_ID 作为 model
        messages=messages,
        temperature=temperature,
    )
    return completion.choices[0].message.content
